...
**1. 工具参数里写在对象末尾的键，偶发传不到工具。**
实测中两次把 `id` 写在块对象的最后一个键，工具收到的却是「没有 id」；同一批数据里写在中间的 `id` 正常。因为工具会给漏写的交互块自动补 `q1`/`q2`（和界面上的题号一致），这个具体案例不影响使用。
但如果规律成立，`required`、`selectable`、`multiple` 这类写在末尾的字段也可能被静默丢掉 —— **目前没有复现证据**。如果遇到「明明写了必填却没校验」，先怀疑这里。
规避方式：把 `id` / `required` 这类关键字段写在对象靠前的位置，别放最后。

**2. macOS / Linux 的浏览器探测代码已写好，但只在 Windows 11 + Edge / Chrome 上实测过。**
探测逻辑覆盖 Chrome / Edge / Brave / Chromium 与 `xdg-open` / `open` 回退，欢迎反馈。

**3. 改 `tools/*.ts` 必须重启 opencode 才生效。**
工具是启动时载入内存的；`assets/ui.html` 则是每次弹窗前现读，改界面不用重启。
